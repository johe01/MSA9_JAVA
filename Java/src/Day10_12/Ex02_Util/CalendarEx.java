...
//		new Calendar() 대신, getInstance() 메소드로 객체를 사용한다.
		...
		printCalendar("자바 마지막 수업일(연/일/월)", test);
		...
		birth.set(2001, Calendar.OCTOBER, 19);				// 내 생일로 날짜 지정하기
		long birthD = birth.getTimeInMillis()/(1000 * 60 * 60 * 24);	// 내 생일 ms로 반환하고 일 단위로 환산하기
