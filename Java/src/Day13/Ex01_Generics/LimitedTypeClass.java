...
public class LimitedTypeClass {
	...
	public static void main(String[] args) {
		...
		// X 클래스로 타입 매개변수 지정 불가 (X는 A보다 상위 클래스임)
		...
		d1.setT(new C());		// B의 부모 클래스인 A클래스로 업캐스팅
		
		// d2<B>, d3<C> <-- 부모 객체인 A 대입 불가 (먼저 업캐스팅이 된 상태였어야 함)
//		(에러) d2.setT(new A());		
//		(에러) d3.setT(new A());
		
		// * 타입 지정을 생략한 경우,
		// 제네릭 타입을 제한한 A 클래스를 기본으로 타입지정하여 생성한다.
		D d5 = new D();		// D<A> d5 = new<A>();
		d5.setT(new B());
		d5.setT(new C());
